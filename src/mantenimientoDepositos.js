...
/* ******************       LOADING TOTALS  ********* */
function loadDeposits (inicioF, finalF) {
    const transaction = new sql.Transaction( pool1 );
    transaction.begin(err => {
        var rolledBack = false;
        transaction.on('rollback', aborted => {
        ...
        const request = new sql.Request(transaction);
        request.query("select * from Depositos where fecha between '"+formatDateCreation(inicioF)+"' and '"+formatDateCreation(finalF)+"'", (err, result) => {
            if (err) {
                if (!rolledBack) {
                    transaction.rollback(err => {
                        $("body").overhang({
...
$('#fechaInicioDepositos').datepicker({
    format: "dd-mm-yyyy",
    todayHighlight: true,
    viewMode: "days", 
    minViewMode: "days",
...
$('#fechaFinalDepositos').datepicker({
    format: "dd-mm-yyyy",
    todayHighlight: true,
    viewMode: "days", 
    minViewMode: "days",
...
function renderTable () {
    if ( $.fn.dataTable.isDataTable( '#datatable_depositos' ) )
        $("#datatable_depositos").dataTable().fnDestroy();
    $( "#datatable_depositos tbody").unbind( "click" );
    var table = $('#datatable_depositos').DataTable({
        "data": arregloActivos,
        dom: "Bflrtip",
        buttons: [
            {
    ...
    $('#datatable_depositos tbody').on( 'click', 'tr a.deleteVariable', function () {
...
function deleteVariable (row) {
    ...
    transaction.begin(err => {
        ...
        const request = new sql.Request(transaction);
        request.query("delete from Depositos where ID = '"+row.ID+"' ", (err, result) => {
            if (err) {
                if (!rolledBack) {
                    transaction.rollback(err => {
                        $("body").overhang({
                ...
                transaction.commit(err => {
                    ...
                    var fechaInicio = $("#fechaInicioDepositos").datepicker('getDate');
                    var fechaFinal = $("#fechaFinalDepositos").datepicker('getDate');
                        ...
                        if (isNaN(fechaInicio.getTime())) {
                            ...
                            if (Object.prototype.toString.call(fechaFinal) === "[object Date]") {
                                if (isNaN(fechaFinal.getTime())) {
                                    ...
                                    loadDeposits(fechaInicio, fechaFinal);
                                }
                            } else {
                                var hoy = new Date();
                                loadDeposits(hoy, hoy);
